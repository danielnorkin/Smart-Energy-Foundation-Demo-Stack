...
function GetLikelyLocationsForTemplate($templateResourceTypesSet)
{
    ...
    Foreach($resourceType in $templateResourceTypesSet)
    {
        ...
        $locs = ((Get-AzResourceProvider -ProviderNamespace $resNamespace).ResourceTypes | Where-Object ResourceTypeName -eq $resName).Locations
...
function Get-AccessToken($tenantId) {
    ...
    {
        if(-not (Get-Module AzRm.Profile)) {
            Import-Module AzRm.Profile
        }
...
function SelectSubscription()
{
    ...
    if($input -eq "0")
    {
        ...
        Get-AzSubscription -SubscriptionId $selectedSubId | Select-AzSubscription 
